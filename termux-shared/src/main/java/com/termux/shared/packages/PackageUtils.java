...
public class PackageUtils {
    ...
    /**
     * Get the {@link ApplicationInfo} for the given {@code packageName}.
     *
     * This is a helper kept for compatibility with code copied from the original termux-api
     * project, which expects {@code PackageUtils.getApplicationInfoForPackage(...)} to exist.
     *
     * @param context The {@link Context} for operations.
     * @param packageName The package name whose {@link ApplicationInfo} to get.
     * @return Returns the {@link ApplicationInfo} or {@code null} if it cannot be resolved.
     */
    @Nullable
    public static ApplicationInfo getApplicationInfoForPackage(@NonNull final Context context, @NonNull String packageName) {
        try {
            PackageManager packageManager = context.getPackageManager();
            if (packageManager == null) return null;
            return packageManager.getApplicationInfo(packageName, 0);
        } catch (final Exception e) {
            Logger.logStackTraceWithMessage(LOG_TAG, "Failed to get ApplicationInfo for package \"" + packageName + "\"", e);
            return null;
        }
    }
