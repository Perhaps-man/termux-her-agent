...
public class SmsInboxAPI {
    ...
    private static void writeElement(Cursor c, JsonWriter out, Map<String, String> nameCache, Context context) throws IOException {
        ...
        int idxThread = c.getColumnIndex(THREAD_ID);
        ...
        if (idxThread < 0 || idxAddress < 0 || idxBody < 0 || idxDate < 0 || idxId < 0 || idxType < 0) {
            throw new IOException("Missing required SMS column");
        }

        int threadID = c.getInt(idxThread);
        String smsAddress = c.getString(idxAddress);
        String smsBody = c.getString(idxBody);
        if (smsAddress == null) smsAddress = "";
        if (smsBody == null) smsBody = "";
        long smsReceivedDate = c.getLong(idxDate);
        int smsID = c.getInt(idxId);

        String smsSenderName = getContactNameFromNumber(nameCache, context, smsAddress);
        String messageType = getMessageType(c.getInt(idxType));

        out.beginObject();
        out.name("threadid").value(threadID);
        out.name("type").value(messageType);

        int index = c.getColumnIndex(READ);
        if (index >= 0) {
        ...
        String receivedStr;
        try {
            receivedStr = dateFormat.format(new Date(smsReceivedDate));
        } catch (Throwable t) {
            receivedStr = "";
        }
        out.name("received").value(receivedStr);
        ...
        try (Cursor messageCursor = cr.query(contentURI, null,
            ...
            for (int i = 0; i < messageCount; i++) {
                try {
                    ...
                    Logger.logStackTraceWithMessage(LOG_TAG, "writeElement failed for message index " + i, t);
                    try {
                        out.beginObject();
                        out.name("_id").value(-1);
                        out.name("body").value("[parse error: " + t.getMessage() + "]");
                        out.name("type").value("inbox");
                        out.name("address").value("");
                        out.name("received").value("");
                        out.endObject();
                    } catch (IOException e2) {
                        Logger.logStackTraceWithMessage(LOG_TAG, "Failed to write error placeholder", e2);
                    }
                }
                if (messageReturnNoOrderReverse) {
                    messageCursor.moveToNext();
    ...
    private static String getContactNameFromNumber(Map<String, String> cache, Context context, String number) {
        ...
        try {
            ...
            Logger.logStackTraceWithMessage(LOG_TAG, "getContactNameFromNumber failed", t);
            cache.put(number, null);
            return null;
        }
    }
