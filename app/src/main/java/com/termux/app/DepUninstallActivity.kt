...
/**
 * 依赖卸载页：按顺序执行 `pkg uninstall` 卸载选中的依赖。
 */
class DepUninstallActivity : AppCompatActivity() {
    ...
    companion object {
        ...
        fun createIntent(context: android.content.Context, deps: List<DepInfo>): android.content.Intent {
            return android.content.Intent(context, DepUninstallActivity::class.java).apply {
        ...
        lifecycleScope.launch {
            ...
            deps.forEachIndexed { index, dep ->
                ...
                withContext(Dispatchers.Main) {
                    setRowRunning(row, "卸载中")
                }
                scrollToRow(index)
                val exitCode = runCatching {
                    execCommand("pkg uninstall -y ${dep.pkgName}", sessionId = "dep_uninstall") { out ->
    ...
    private fun setRowRunning(row: DepRowView, label: String) {
        row.dot.text = "●"
    ...
    private fun retryFailed() {
        val failedIndices = rowViews.mapIndexed { index, row -> index to row }
            .filter { (_, row) -> row.status.text == "卸载失败" }
            .map { it.first }
        ...
        lifecycleScope.launch {
            ...
            failedIndices.forEachIndexed { relIndex, absIndex ->
                ...
                withContext(Dispatchers.Main) { setRowRunning(row, "卸载中") }
    ...
    private fun scrollToRow(index: Int) {
        scrollView.post {
            val row = rowViews.getOrNull(index) ?: return@post
            scrollView.smoothScrollTo(0, row.root.top)
        }
    }
}
